...
hbb_name = 'HangtingZuoxi' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/HangtingZuoxi.exe')
    os.chdir('../..')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/HangtingZuoxi.exe target/release/HangtingZuoxi.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\HangtingZuoxi.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/HangtingZuoxi.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/HangtingZuoxi-install.exe')
        system2('mv ../../{res_dir}/HangtingZuoxi-install.exe ../..')
